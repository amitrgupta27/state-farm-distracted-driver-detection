...
count = 0
source = "/home/ec2-user/Mask_RCNN/imgs/train/c"
for i in range(10):
    
    print('now we are in the folder C',i)
    imgs = os.listdir(source+str(i))
    print(f'number of imgs = {len(imgs)}, first image ={imgs[0]}')
    for j in range(len(imgs)):
    #for j in range(1000):

        img_name = source+str(i)+"/"+imgs[j]
        ...
        dest_path = './Mask_RCNN/mask/train/c'+str(i)+"/"+imgs[j]
        ...
        if (found_person):
            ...
            # Mask RCNN didn't person. Perform a manual zoom
            temp = temp[50:,120:-50]
